...
use crate::app::{App, AppResult, ProcessSortColumn, ProcessViewState, SelectedTab, SortDirection};
    ...
    match app.selected_tab {
        ...
        SelectedTab::Process => {
            match key_event.code {
                // Basic Navigation & Actions (no changes)
                        ...
                        if let Some(table_state) = app.table_states.get_mut(&app.selected_tab) {
                            ...
                            if amount_items == 0 {
                                ...
                                if selected > 0 {
                                    ...
                                    table_state.select(Some(amount_items - 1)); // Wrap to bottom
                                }
                            } else {
                                // Nothing selected, select the last item
                                table_state.select(Some(amount_items - 1));
                            }
                        }
                    }
